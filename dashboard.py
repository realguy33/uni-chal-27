...
st.write("Uniswap is DEX(Decentrailized Exchange) on Ethereum mainnet and it is also available on various L2s such Arbitrum,Optimism and polygon. The protocol uses a set of smart contracts to create liquidity pools and swap assets. Reserves and prices are updated every time a trade is made by way of a constant product formula. All of these eliminate the need for central intermediaries.")
st.write("The advantage of using L2s over the ethereum mainnet is that the gas fees on L2s are much much lower than ethereum and these L2s also claim to have the same security as that  of ethereum. This makes them attractive to the users.This means that the usage of various L2s should slowly be increasing. is this true? let's find out:")
